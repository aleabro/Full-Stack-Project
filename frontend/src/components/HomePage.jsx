...
export default function HomePage({ events, filteredEvents, user, organizations}) {
  ...
  const [isNewsletterSubscribed, setIsNewsletterSubscribed] = useState(user?.newsletter);
  const location = useLocation();
  
  // Stati separati per l'ordinamento
  ...
  useEffect(() => {
    setIsNewsletterSubscribed(user?.newsletter);
  }, [user]);
...
useEffect(() => {
  ...
  try {
    ...
    console.log("Risposta API:", response.status, data); // <--- AGGIUNGI QUESTO
    if (response.ok) {
